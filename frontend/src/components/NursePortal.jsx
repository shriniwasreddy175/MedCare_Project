...
import {
    Users, Bell, ArrowUp, MapPin, LogOut, Shield, Heart, Activity, Droplets, Thermometer, AlertTriangle, Phone,
    Clock, TrendingUp, Plus, MessageCircle, Stethoscope, Baby, ChevronRight, Search
} from 'lucide-react';

const API_URL = 'https://medcare-api-i5cm.onrender.com/api';

export default function NursePortal({ onLogout, userName }) {
    ...
    useEffect(() => {
        ...
        let tabFiltered;
        if (activeTab === 'alerts') {
            tabFiltered = newFilteredPatients.filter(p => p.caseType === 'New Alert');
        } else if (activeTab === 'escalated') {
            tabFiltered = newFilteredPatients.filter(p => p.caseType === 'Escalated');
        } else { // 'patients' tab
            tabFiltered = newFilteredPatients;
        }
        setFilteredPatients(tabFiltered);

    }, [searchQuery, activeTab, patients]);

    // Derived stats from the 'patients' state
    const criticalAlerts = patients.filter(p => p.caseType === 'New Alert').length;
    const escalatedAlerts = patients.filter(p => p.caseType === 'Escalated').length;
    const stablePatients = patients.filter(p => p.caseType === 'Active').length;
    ...
    const handleEscalateToDoctor = async (patient) => {
        ...
        try {
            ...
            setStatusMessage("Network error during escalation.");
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="patient-detail-panel">
            <div className="patient-vitals-section expanded-vitals">
                <div className="vital-item">
                    <div className="vital-value-wrapper"><Heart className="icon-sm text-red-600" /><span className="vital-value">{patient.vitals.heartRate}</span></div>
                    <div className="vital-label">Heart Rate (HR)</div>
                </div>
                <div className="vital-item">
                    <div className="vital-value-wrapper"><Activity className="icon-sm text-blue-600" /><span className="vital-value">{patient.vitals.bloodPressure}</span></div>
                    <div className="vital-label">Blood Pressure (BP)</div>
                </div>
                <div className="vital-item">
                    <div className="vital-value-wrapper"><Droplets className="icon-sm text-cyan-600" /><span className="vital-value">{patient.vitals.spo2}</span></div>
                    <div className="vital-label">SpO2</div>
                </div>
                <div className="vital-item">
                    <div className="vital-value-wrapper"><Thermometer className="icon-sm text-orange-600" /><span className="vital-value">{patient.vitals.temperature}</span></div>
                    <div className="vital-label">Temp</div>
                </div>
            </div>

            <div className="patient-actions expanded-actions">
                <button className="button button-outline-sm" disabled={loading}>
            ...
            <div className="nurse-sidebar">
                    ...
                    <nav className="nurse-nav">
                        {sidebar.map((item) => (
                            ...
                            >
                                ...
                                {item.count && !item.badge && ( <div className="badge badge-outline">{item.count}</div> )}
                            </button>
                        ))}
                    </nav>
                ...
                <div className="nurse-profile-section">
                    <div className="profile-info">
                        <div className="profile-avatar-bg"> <span className="profile-avatar-text">{userName ? userName[0].toUpperCase() : 'N'}</span> </div>
                        <div>
                            <div className="profile-name">{userName || 'Nurse'}</div>
            ...
            <div className="nurse-main-content">
                <div className="main-header">
                    <div>
                        ...
                        <p className="main-subtitle"> {currentTime.toLocaleDateString()} • {currentTime.toLocaleTimeString()} </p>
                    </div>
                ...
                <div className="patient-list-container">
                    {loading && filteredPatients.length === 0 ? (
                        <div className="empty-state-message">Loading patient data...</div>
                        ...
                        filteredPatients.map((patient) => (
                            <React.Fragment key={patient.id}>
                                <div
                                    className={`patient-list-row ${patient.hasAlert ? (patient.alertLevel === 'doctor' ? 'alert-row-escalated' : 'alert-row') : ''} ${selectedPatientId === patient.id ? 'expanded' : ''}`}
                ...
                <div className="dialog-overlay" onClick={() => setManualEntryOpen(false)}>
                    <form className="dialog-content" onSubmit={handleSaveEntry} onClick={e => e.stopPropagation()}>
                        <div className="dialog-header">
                            ...
                            {entryStatus && <p className={`status-message ${entryStatus.startsWith('Error') ? 'error' : 'success'}`}>{entryStatus.replace(/^(Success: |Error: )/, '')}</p>}
                        </div>
                        <div className="dialog-body">
                            <div>
                                <label className="input-label">Patient Name (Required)</label>
                                <input className="input-field" placeholder="Enter full patient name (e.g., Patient Alpha)" name="patient_name" value={manualFormData.patient_name} onChange={handleFormChange} required disabled={loading} />
                            </div>
                            <div className="input-grid">
                                <div>
                                    <label className="input-label">Heart Rate (bpm) (Required)</label>
                                    <input className="input-field" placeholder="72 bpm" name="heart_rate" value={manualFormData.heart_rate} onChange={handleFormChange} required disabled={loading} />
                                </div>
                                <div>
                                    <label className="input-label">Blood Pressure (mmHg)</label>
                                    <input className="input-field" placeholder="120/80 mmHg" name="blood_pressure" value={manualFormData.blood_pressure} onChange={handleFormChange} disabled={loading} />
                                </div>
                            </div>
                            <div className="input-grid">
                                <div>
                            ...
                            <div>
                                ...
                                <textarea className="textarea-field" placeholder="Additional observations..." name="notes" value={manualFormData.notes} onChange={handleFormChange} disabled={loading}></textarea>
                            </div>
