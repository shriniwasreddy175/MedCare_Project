...
// NOTE: The mock list is completely removed. Data will be fetched from the API.

export default function DoctorPortal({ onLogout, userName }) { // RECEIVING userName PROP
...
            
            // Enhance data with case type definitions for client-side filtering
            const processedData = data.map(p => ({
                ...p,
                // Logic assumes API provides alertLevel: 'doctor' or 'nurse'
...
    // Filtering logic runs whenever patients or searchQuery changes
    useEffect(() => {
...
        // NOTE: This is a mock action and should send an API call to mark the case as resolved.
        alert(`Case for Patient ${patientId} acknowledged by ${userName}. Updating records...`);
        
        // Mock client-side update: moves patient to active list
        setPatients(prev => prev.map(p => 
            p.id === patientId ? { ...p, hasAlert: false, caseType: 'Active', alertLevel: 'none', escalatedBy: null } : p
        ));
    };

    const handleAddNotes = (patientName) => {
        // Uses the actual logged-in doctor's name
        alert(`Opening EMR form to add notes for ${patientName} by ${userName}.`);
    };
...
                <div className="empty-state-message">No patients found in the **{activeTab.replace('_', ' ')}** view.</div>
            ) : (
...
                                {/* DISPLAY ESCALATION SOURCE */}
                                {patient.caseType === 'Escalated' && patient.escalatedBy && (
                                    <p className="escalation-source">
                                        <ArrowUp size={14} className="escalate-icon" /> Escalated by: **{patient.escalatedBy}**
                                    </p>
                                )}
                            </div>
                        </div>
...
                        </div>
                        <div>
                            <div className="profile-name">{userName || 'Doctor'}</div> {/* Displays actual name */}
...
                            <div className="stat-card-doctor"> <div className="stat-card-content"> <div><p className="stat-label">Stable</p> <p className="stat-value-green">{totalActive - escalatedCount - newCasesCount}</p></div> <TrendingUp className="stat-icon-green" /> </div> </div>
                        </div>
